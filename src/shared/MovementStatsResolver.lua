...
function Resolver.Resolve(model: Model, state: any): any
	local pace = math.clamp(tonumber(model:GetAttribute("PAC")) or 60, 1, 99)
	local acceleration = math.clamp(tonumber(model:GetAttribute("Acceleration")) or pace, 1, 99)
	...
	local jog = Tuning.JogMin + (Tuning.JogMax - Tuning.JogMin) * (pace / 99)
	local sprint = Tuning.SprintMin + (Tuning.SprintMax - Tuning.SprintMin) * (pace / 99)
	...
	local accelerationRate = Tuning.AccelerationMin + (Tuning.AccelerationMax - Tuning.AccelerationMin) * (acceleration / 99)
