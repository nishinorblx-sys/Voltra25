...
function ClubIdentityService:Create(player:Player,name:string,tag:string):boolean local p=self.Profiles:GetProfile(player);if not p or p.ClubMembership.ClubId~="" or type(name)~="string" or #name<3 or #name>24 or type(tag)~="string" or #tag<2 or #tag>5 then return false end;local cleanTag=string.upper(tag);local club=copy(p.ClubMembership or{});for key,value in Templates.NewClub do if club[key]==nil or club[key]==""then club[key]=copy(value)end end;club.ClubId="club_"..player.UserId;club.Name=string.upper(name);club.Abbreviation=cleanTag;club.Tag=cleanTag;club.Role=club.Role=="FREE AGENT"and"FOUNDER"or club.Role;club.Members=math.max(1,tonumber(club.Members)or 0);club.Capacity=tonumber(club.Capacity)or 24;club.Reputation=club.Reputation=="UNRANKED"and"ROOKIE"or club.Reputation;p.ClubMembership=club;p.Profile.SelectedClub=club.Name;return true end
