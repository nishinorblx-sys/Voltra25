...
local function cardInstructions(profile:any,card:any,role:any,slot:any):any
	local instructions=type(profile.PlayerInstructions)=="table"and profile.PlayerInstructions or{}
	local cardId=tostring(card and(card.cardInstanceId or card.Id)or"")
	local value=instructions[cardId]
	if type(value)=="table"then return AIPlayerInstructionConfig.Normalize(value,role)end
	if type(instructions[slot])=="string"then return AIPlayerInstructionConfig.FromLegacyProfile(instructions[slot],role)end
	return AIPlayerInstructionConfig.RoleDefaults(role)
end
