...
function CurrencyService:GetClientData(player: Player): any?
	...
	return { Coins = profile.Currency.Coins, Bolts = profile.Currency.Bolts }
end
...
function CurrencyService:Add(player: Player, currency: string, amount: number): boolean
	if (currency ~= "Coins" and currency ~= "Bolts") or type(amount) ~= "number" or amount % 1 ~= 0 or amount <= 0 or amount > 100000 then return false end
	local profile = self.Profiles:GetProfile(player)
