...
local StoreService={};StoreService.__index=StoreService
local function find(list:any,id:string):any? for _,item in list do if item.Id==id then return item end end;return nil end
local function contains(list:any,id:string):boolean return table.find(list,id)~=nil end
function StoreService.new(profiles:any,inventory:any) return setmetatable({Profiles=profiles,Inventory=inventory,LastPurchase={}},StoreService) end
function StoreService:Purchase(player:Player,kind:string,id:string,quantity:number?):(boolean,string)
	local p=self.Profiles:GetProfile(player);if not p then return false,"Profile unavailable." end;local now=os.clock();if now-(self.LastPurchase[player] or 0)<.35 then return false,"Please wait." end
	local item,bucket;if kind=="Pack" then item=Catalog.Packs[id] elseif kind=="Kit" then item=find(Catalog.Kits,id);bucket=p.StoreOwnership.Kits elseif kind=="StadiumTheme" then item=find(Catalog.StadiumThemes,id);bucket=p.StoreOwnership.Stadiums elseif kind=="Cosmetic" then item=find(Catalog.Cosmetics,id);bucket=p.StoreOwnership.Cosmetics end
