...
function Service.new(profiles:any,inventory:any,squad:any)
	return setmetatable({Profiles=profiles,Inventory=inventory,Squad=squad,Listings=DataStoreService:GetDataStore("VTR25_TransferListings_v1"),Index=DataStoreService:GetOrderedDataStore("VTR25_TransferIndex_v1"),Mail=DataStoreService:GetDataStore("VTR25_TransferMail_v1"),Local={},Token=game.JobId~=""and game.JobId or HttpService:GenerateGUID(false)},Service)
end
...
function Service:_settle(listing:any)
	...
	pcall(function()self.Index:RemoveAsync(listing.ListingId)end);self.Local[listing.ListingId]=nil
end
function Service:Start()
	...
	task.spawn(function()while task.wait(5)do for _,listing in self.Local do self:_settle(listing)end;local ok,pages=pcall(function()return self.Index:GetSortedAsync(true,40,nil,os.time())end);if ok and pages then for _,entry in pages:GetCurrentPage()do local loaded,value=pcall(function()return self.Listings:GetAsync(entry.key)end);if loaded then self:_settle(value)end end end end end)
end
function Service:_write(id:string,listing:any):boolean local ok=pcall(function()self.Listings:SetAsync(id,listing);self.Index:SetAsync(id,listing.EndsAt)end);if not ok then self.Local[id]=listing end;return true end
...
function Service:PlaceBid(player:Player,listingId:string,amount:number):(boolean,string,any?)
	...
	if not DeveloperConfig.InfiniteCoinsEveryone then profile.Currency.Coins-=amount end;pcall(function()self.Index:SetAsync(listingId,listing.EndsAt)end)
