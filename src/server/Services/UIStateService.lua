...
function UIStateService:Start()
	self.Remote.OnServerEvent:Connect(function(player,payload)
		...
		local now=os.clock();if now-(self.LastUpdate[player] or 0)<.05 then return end;self.LastUpdate[player]=now
		local profile=self.Profiles:GetProfile(player);if not profile then return end;local state=profile.UIState;local kind=payload.Type
