...
function Service:RecordShot(model:Model,onTarget:boolean,xg:number)
	...
	if root then local p=self.PitchCFrame:PointToObjectSpace(root.Position);table.insert(self.ShotMap[team],{From={X=p.X,Z=p.Z},Shooter=model:GetAttribute("DisplayName"),OnTarget=onTarget,XG=math.floor(xg*100+.5)/100})end
...
function Service:Serialize(homeScore:number,awayScore:number,gameSeconds:number?):any
	...
	local function team(v:any)local attempts=math.max(1,v.Passes);local dribbles=math.max(1,v.Dribbles);return{Possession=math.floor(v.Possession/total*100+.5),Shots=v.Shots,ShotsOnTarget=v.ShotsOnTarget,ShotsOffTarget=math.max(0,v.Shots-v.ShotsOnTarget),Goals=v.Goals,ExpectedGoals=math.floor(v.ExpectedGoals*100+.5)/100,PassesAttempted=v.Passes,PassesCompleted=v.CompletedPasses,PassAccuracy=math.floor(v.CompletedPasses/attempts*100+.5),TacklesAttempted=v.Tackles,TacklesCompleted=v.CompletedTackles,Interceptions=v.Interceptions,Saves=v.Saves,Corners=v.Corners,Fouls=v.Fouls,Offsides=v.Offsides,YellowCards=v.YellowCards,RedCards=v.RedCards,CornersIntoBox=v.CornersIntoBox,CornerReachedTeammate=v.CornerReachedTeammate,CornerGoals=v.CornerGoals,BlockedShots=v.Blocks,Blocks=v.Blocks,Clearances=v.Clearances,Crosses=v.Crosses,CompletedCrosses=v.CompletedCrosses,Dribbles=v.Dribbles,DribblesCompleted=v.CompletedDribbles,DribbleAccuracy=math.floor(v.CompletedDribbles/dribbles*100+.5),KeyPasses=v.KeyPasses,BigChanceCreated=v.BigChancesCreated,Errors=v.Errors}end
