...
function Service.Evaluate(context: any, shooter: any, style: any, difficulty: any): any
	...
	local closeChance = insideBox and distance < 72
	local longShotAllowed = style:Ratio("LongShotFrequency") > 0.55 and shooter.Stats.longShots >= 72
	local good = closeChance or (insideBox and distance < 115 and clearAngle and not pressure.Heavy) or (central and distance < 150 and shooter.Stats.shooting >= 75 and clearAngle) or oneVOne
	if edge and longShotAllowed and clearAngle and not pressure.Heavy then
		good = true
	end
	local bad = distance > 210 or wideAngle or (pressure.Heavy and not closeChance) or (not insideBox and shooter.Stats.shooting < 55)
	local score = 0
	score += insideBox and 42 or central and 28 or edge and 12 or -10
