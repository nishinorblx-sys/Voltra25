...
function ServerApp.Start()
	...
	-- v8 intentionally starts every account from a fresh profile after the full data reset.
	-- Bump this only when another full live data reset is desired.
	local profiles = ProfileService.new(MockProfileStore.new(DefaultProfile, "VTR25_LaunchProfiles_v8"))
	...
	squadAction.OnServerInvoke=function(player:Player,action:any,payload:any)
		...
		local ok,success,message,completed,data=pcall(function()
			...
			elseif action=="SetPlayerInstructions" then local accepted,text,data=squad:SetPlayerInstructions(player,payload.CardInstanceId,payload.OffBall,payload.Defending);return accepted,text,false,data
