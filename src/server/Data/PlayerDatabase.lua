...
for index,player in players do byId[player.playerId]=player;pools[player.rarity]=pools[player.rarity]or{};table.insert(pools[player.rarity],player);typePools[player.cardType]=typePools[player.cardType]or{};table.insert(typePools[player.cardType],player);typedPools[player.cardType]=typedPools[player.cardType]or{};typedPools[player.cardType][player.rarity]=typedPools[player.cardType][player.rarity]or{};table.insert(typedPools[player.cardType][player.rarity],player);add(indexes.country,string.lower(player.country),player);add(indexes.club,string.lower(player.club),player);add(indexes.rarity,string.lower(player.rarity),player);add(indexes.cardType,string.lower(player.cardType),player);for _,position in player.positions do add(indexes.position,string.lower(position),player)end;if index%500==0 then task.wait() end end
