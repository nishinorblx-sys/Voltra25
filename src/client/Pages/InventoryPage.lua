...
function Page.new(context:any):CanvasGroup
	...
	local function renderPlayers()
		...
		VirtualizedList.new(body,{Position=UDim2.fromOffset(0,50),Size=UDim2.new(1,0,1,-50),Items=players,RowHeight=133,Gap=9,Buffer=10,RenderItem=function(parent:Instance,card:any):GuiObject local row=Panel.new({Name=card.Id});row.Parent=parent;local wide=WidePlayerCard.new({Parent=row,Card=card,Size=UDim2.new(1,-310,0,108),Meta=card.Meta,OnActivated=function()context.OpenPlayerDetails(card.Id)end});wide.Position=UDim2.fromOffset(8,8);local details=Button.new({Text="VIEW DETAILS",Variant="Secondary",Size=UDim2.fromOffset(132,38),OnActivated=function()context.OpenPlayerDetails(card.Id)end});details.Position=UDim2.new(1,-286,0,20);details.Parent=row;local squad=Button.new({Text="SEND TO SQUAD",Variant="Primary",Size=UDim2.fromOffset(132,38),OnActivated=function()local moved=SquadService:MovePlayer(card.Id,"Bench",1);toast(moved.Message or(moved.Success and"Player sent to squad."or"Squad move failed."),moved.Success and"Info"or"Error")end});squad.Position=UDim2.new(1,-146,0,66);squad.Parent=row;return row end})
	end
