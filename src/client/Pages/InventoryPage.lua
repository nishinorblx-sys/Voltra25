...
function Page.new(context:any):Frame
	...
	local availableTabs=if packsUnlocked then TABS else {"Players","Club Items"}
	local activeTab=(context.Data.UIState.SelectedTabs and context.Data.UIState.SelectedTabs.Inventory) or (packsUnlocked and "Packs"or"Players");if not table.find(availableTabs,activeTab)then activeTab=packsUnlocked and"Packs"or"Players"end;local search="";local positionIndex=1;local rarityIndex=1;local highFirst=true;local bulkSellMode=false;local bulkSelected={};local playersScrollY=0
	...
	local function quickSellValue(card:any):number return QuickSellValueConfig.Value(card,type(card)=="table"and card.Meta or nil)end
	local function protectionReason(card:any):string?
		local meta=type(card.Meta)=="table"and card.Meta or{}
		if meta.Favorite==true or card.Favorite==true then return "Unfavorite this player before quick selling."end
		if meta.Locked==true or card.Locked==true then return "Unlock this player before quick selling."end
		if meta.Loan==true or card.Loan==true then return "Loan players cannot be quick sold."end
		if meta.CampaignProjectActive==true then return "Retire this Club Project before changing its protected status."end
		if card.CampaignBound==true or card.QuickSellBlocked==true or meta.CampaignBound==true then return "Campaign reward players are account-bound."end
		return nil
	end
	...
	local function renderPlayers()
		local controls=Instance.new("Frame");controls.BackgroundTransparency=1;controls.Size=UDim2.new(1,0,0,84);controls.Parent=body
		...
		local pos=Button.new({Text="POS: "..POSITIONS[positionIndex],Variant="Secondary",Size=UDim2.new(.18,-5,0,36),OnActivated=function()positionIndex=positionIndex%#POSITIONS+1;render()end});pos.Position=UDim2.new(.4,5,0,0);pos.Parent=controls;local rarity=Button.new({Text="RARITY: "..RARITIES[rarityIndex],Variant="Secondary",Size=UDim2.new(.24,-5,0,36),OnActivated=function()rarityIndex=rarityIndex%#RARITIES+1;render()end});rarity.Position=UDim2.new(.58,10,0,0);rarity.Parent=controls;local sort=Button.new({Text=highFirst and "OVR HIGH" or "OVR LOW",Variant="Secondary",Size=UDim2.new(.18,-5,0,36),OnActivated=function()highFirst=not highFirst;render()end});sort.Position=UDim2.new(.82,15,0,0);sort.Parent=controls;local selectedTotal,selectedCount=bulkSelectedTotal();local bulk=Button.new({Text=bulkSellMode and "BULK SELL: ON"or"BULK SELL",Variant=bulkSellMode and"Primary"or"Secondary",Size=UDim2.fromOffset(136,34),OnActivated=function()bulkSellMode=not bulkSellMode;if not bulkSellMode then bulkSelected={}end;render()end});bulk.Position=UDim2.fromOffset(0,44);bulk.Parent=controls;local selectedLabel=label(controls,selectedCount.." SELECTED  /  "..selectedTotal.." COINS",UDim2.fromOffset(148,44),UDim2.new(1,-470,0,34),10,bulkSellMode and Theme.Colors.Electric or Theme.Colors.Muted,Theme.Fonts.Strong);selectedLabel.TextYAlignment=Enum.TextYAlignment.Center;local clear=Button.new({Text="CLEAR",Variant="Secondary",Size=UDim2.fromOffset(92,34),OnActivated=function()bulkSelected={};render()end});clear.Position=UDim2.new(1,-254,0,44);clear.Parent=controls;local sellBulk=Button.new({Text="SELL SELECTED",Variant="Primary",Size=UDim2.fromOffset(150,34),OnActivated=function()local ids=bulkSelectedIds();local total,count=bulkSelectedTotal();if #ids<=0 then toast("Select players to bulk quick sell.","Error");return end;context.Flow:Confirmation("BULK QUICK SELL","Permanently quick sell "..count.." selected players for "..total.." coins total.","SELL SELECTED",function()local sold=SquadService:BulkQuickSellCards(ids);toast(sold.Message or(sold.Success and"Players quick sold."or"Bulk quick sell failed."),sold.Success and"Reward"or"Error");if sold.Success then bulkSelected={};bulkSellMode=false end;refresh();render()end)end});sellBulk.Position=UDim2.new(1,-154,0,44);sellBulk.Parent=controls
		...
		local playerList=VirtualizedList.new(body,{Position=UDim2.fromOffset(0,94),Size=UDim2.new(1,0,1,-94),Items=players,RowHeight=133,Gap=9,Buffer=10,RenderItem=function(parent:Instance,card:any):GuiObject
			local row=Panel.new({Name=card.Id});row.Parent=parent
			local reason=protectionReason(card)
			local selected=bulkSelected[tostring(card.Id)]==true
			local function toggleSelected()
				if not bulkSellMode then context.OpenPlayerDetails(card.Id);return end
				if reason then toast(reason,"Error");return end
				bulkSelected[tostring(card.Id)]=not selected
				render()
			end
			local wide=WidePlayerCard.new({Parent=row,Card=card,Size=UDim2.new(1,-450,0,108),Meta=card.Meta,Selected=selected,OnActivated=toggleSelected});wide.Position=UDim2.fromOffset(8,8)
			if bulkSellMode then
				local pick=Button.new({Text=reason and"PROTECTED"or selected and"SELECTED"or"SELECT",Variant=selected and not reason and"Primary"or"Secondary",Size=UDim2.fromOffset(132,34),OnActivated=toggleSelected});pick.Position=UDim2.new(1,-426,0,16);pick.Parent=row
				label(row,reason and"PROTECTED"or quickSellValue(card).." COINS",UDim2.new(1,-426,0,58),UDim2.fromOffset(132,18),8,selected and Theme.Colors.Electric or Theme.Colors.Muted,Theme.Fonts.Strong)
			else
				local details=Button.new({Text="VIEW DETAILS",Variant="Secondary",Size=UDim2.fromOffset(132,34),OnActivated=function()context.OpenPlayerDetails(card.Id)end});details.Position=UDim2.new(1,-426,0,16);details.Parent=row
				local squad=Button.new({Text="SEND TO SQUAD",Variant="Primary",Size=UDim2.fromOffset(132,34),OnActivated=function()local moved=SquadService:MovePlayer(card.Id,"Bench",1);toast(moved.Message or(moved.Success and"Player sent to squad."or"Squad move failed."),moved.Success and"Info"or"Error");refresh();render()end});squad.Position=UDim2.new(1,-286,0,55);squad.Parent=row
				local sell=Button.new({Text=reason and"PROTECTED"or"QUICK SELL",Variant="Secondary",Size=UDim2.fromOffset(132,34),OnActivated=function()
					if reason then toast(reason,"Error");return end
					context.Flow:Confirmation("QUICK SELL "..string.upper(tostring(card.Name or"PLAYER")),"This permanently removes the card for "..quickSellValue(card).." coins.","QUICK SELL",function()local sold=SquadService:QuickSellCard(card.Id);toast(sold.Message or(sold.Success and"Player quick sold."or"Quick sell failed."),sold.Success and"Reward"or"Error");refresh();render()end)
				end});sell.Position=UDim2.new(1,-146,0,55);sell.Parent=row
			end
			return row
		end})
		playerList:GetPropertyChangedSignal("CanvasPosition"):Connect(function()if playerList.Parent then playersScrollY=playerList.CanvasPosition.Y end end)
		task.defer(function()if playerList.Parent then local maxY=math.max(0,playerList.AbsoluteCanvasSize.Y-playerList.AbsoluteWindowSize.Y);playerList.CanvasPosition=Vector2.new(0,math.clamp(playersScrollY,0,maxY))end end)
	end
	...
	local function renderTransferMarket()
		...
		if#listings==0 then empty(list,"NO ACTIVE AUCTIONS","Player listings from the live market will appear here.");return end
