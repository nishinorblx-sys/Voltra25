...
function Page.new(context:any):CanvasGroup
	...
	local activeTab=(context.Data.UIState.SelectedTabs and context.Data.UIState.SelectedTabs.Inventory) or "Packs";if not table.find(TABS,activeTab)then activeTab="Packs"end;local search="";local positionIndex=1;local rarityIndex=1;local highFirst=true;local bulkSellMode=false;local bulkSelected={}
	...
	local function quickSellValue(card:any):number local normalized=math.clamp(((tonumber(card and card.Rating)or 45)-45)/54,0,1);return math.floor(1000+4000*(normalized^1.55)+.5)end
	...
	local function renderPlayers()
		...
		VirtualizedList.new(body,{Position=UDim2.fromOffset(0,94),Size=UDim2.new(1,0,1,-94),Items=players,RowHeight=133,Gap=9,Buffer=10,RenderItem=function(parent:Instance,card:any):GuiObject
			...
			if bulkSellMode then
				...
				label(row,reason and"ACCOUNT-BOUND"or quickSellValue(card).." COINS",UDim2.new(1,-426,0,58),UDim2.fromOffset(132,18),8,selected and Theme.Colors.Electric or Theme.Colors.Muted,Theme.Fonts.Strong)
			else
				...
				local sell=Button.new({Text=reason and"PROTECTED"or"QUICK SELL",Variant="Secondary",Size=UDim2.fromOffset(132,34),OnActivated=function()
					if reason then toast(reason,"Error");return end
					context.Flow:Confirmation("QUICK SELL "..string.upper(tostring(card.Name or"PLAYER")),"This permanently removes the card and adds coins immediately.","QUICK SELL",function()local sold=SquadService:QuickSellCard(card.Id);toast(sold.Message or(sold.Success and"Player quick sold."or"Quick sell failed."),sold.Success and"Reward"or"Error");refresh();render()end)
