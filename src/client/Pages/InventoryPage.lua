...
function Page.new(context:any):CanvasGroup
	...
	local function bulkSelectedTotal():number local lookup={};for _,card in data.Players or{}do lookup[tostring(card.Id)]=card end;local total=0;local count=0;for id,selected in bulkSelected do if selected and lookup[id]then total+=quickSellValue(lookup[id]);count+=1 elseif selected then bulkSelected[id]=nil end end;return total,count end
	...
	local function renderPlayers()
		...
		VirtualizedList.new(body,{Position=UDim2.fromOffset(0,94),Size=UDim2.new(1,0,1,-94),Items=players,RowHeight=133,Gap=9,Buffer=10,RenderItem=function(parent:Instance,card:any):GuiObject local row=Panel.new({Name=card.Id});row.Parent=parent;local selected=bulkSelected[tostring(card.Id)]==true;local function toggleSelected()if not bulkSellMode then context.OpenPlayerDetails(card.Id);return end;bulkSelected[tostring(card.Id)]=not selected;render()end;local wide=WidePlayerCard.new({Parent=row,Card=card,Size=UDim2.new(1,-450,0,108),Meta=card.Meta,Selected=selected,OnActivated=toggleSelected});wide.Position=UDim2.fromOffset(8,8);if bulkSellMode then local pick=Button.new({Text=selected and"SELECTED"or"SELECT",Variant=selected and"Primary"or"Secondary",Size=UDim2.fromOffset(132,34),OnActivated=toggleSelected});pick.Position=UDim2.new(1,-426,0,16);pick.Parent=row;label(row,quickSellValue(card).." COINS",UDim2.new(1,-426,0,58),UDim2.fromOffset(132,18),8,selected and Theme.Colors.Electric or Theme.Colors.Muted,Theme.Fonts.Strong)else local details=Button.new({Text="VIEW DETAILS",Variant="Secondary",Size=UDim2.fromOffset(132,34),OnActivated=function()context.OpenPlayerDetails(card.Id)end});details.Position=UDim2.new(1,-426,0,16);details.Parent=row;local squad=Button.new({Text="SEND TO SQUAD",Variant="Primary",Size=UDim2.fromOffset(132,34),OnActivated=function()local moved=SquadService:MovePlayer(card.Id,"Bench",1);toast(moved.Message or(moved.Success and"Player sent to squad."or"Squad move failed."),moved.Success and"Info"or"Error");refresh();render()end});squad.Position=UDim2.new(1,-286,0,55);squad.Parent=row;local sell=Button.new({Text="QUICK SELL",Variant="Secondary",Size=UDim2.fromOffset(132,34),OnActivated=function()context.Flow:Confirmation("QUICK SELL "..string.upper(tostring(card.Name or"PLAYER")),"This permanently removes the card and adds coins immediately.","QUICK SELL",function()local sold=SquadService:QuickSellCard(card.Id);toast(sold.Message or(sold.Success and"Player quick sold."or"Quick sell failed."),sold.Success and"Reward"or"Error");refresh();render()end)end});sell.Position=UDim2.new(1,-146,0,55);sell.Parent=row end;return row end})
	end
