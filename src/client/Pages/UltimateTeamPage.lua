...
function UltimateTeamPage.new(context:any):CanvasGroup
	...
	local function makeCard(parent:Instance,card:any?,size:UDim2,kind:string,slot:any?,compact:boolean?):TextButton
		...
		if card then
			...
			button=CompactPlayerCard.new({Parent=parent,Card=card,Size=size,Horizontal=compact==true,Selected=selected,Meta=rosterMeta(snapshot,card),ChemistryColor=compact and nil or (outOfPosition and Theme.Colors.Warning or Theme.Colors.White)})
		...
		if pendingCardId and (kind=="StartingXI" or kind=="Bench" or kind=="Reserves") then local targetGlow=Instance.new("UIStroke");targetGlow.Name="MoveDestinationGlow";targetGlow.Color=Theme.Colors.White;targetGlow.Thickness=1;targetGlow.Transparency=.38;targetGlow.Parent=button end
	...
	local function actionMenu()
		...
		text(menu,"PLAYER ACTIONS",UDim2.fromOffset(20,16),UDim2.new(1,-40,0,24),9,Theme.Colors.White,Theme.Fonts.Strong);text(menu,card.Rating.."  "..card.Name.."  /  "..card.Position,UDim2.fromOffset(20,41),UDim2.new(1,-40,0,30),15,Theme.Colors.White,Theme.Fonts.Display)
	...
	renderSummary=function()
		...
		text(teamContent,"TEAM OVERALL",UDim2.fromOffset(0,44),UDim2.new(1,0,0,18),8,Theme.Colors.Muted,Theme.Fonts.Strong);text(teamContent,tostring(snapshot.Rating),UDim2.fromOffset(0,62),UDim2.new(1,0,0,36),25,Theme.Colors.White,Theme.Fonts.Display)
		...
		text(teamContent,"SAVED  "..os.date("!%H:%M:%S",snapshot.SavedAt or os.time()),UDim2.new(0,0,1,-28),UDim2.new(1,0,0,20),7,Theme.Colors.White,Theme.Fonts.Strong)
	end
	...
	openPackHub=function()
		...
		text(hub,"UNOPENED INVENTORY  /  STORE  /  TRANSPARENT ODDS  /  OPEN HISTORY",UDim2.fromOffset(24,50),UDim2.new(1,-48,0,18),8,Theme.Colors.White,Theme.Fonts.Strong).ZIndex=73
	...
	local function openObjectivesHub()
		...
		text(hub,"OBJECTIVES",UDim2.fromOffset(24,16),UDim2.new(1,-150,0,34),22,Theme.Colors.White,Theme.Fonts.Display).ZIndex=73;text(hub,"LIVE SERVER PROGRESS  /  REWARDS  /  LIMITED LOAN PLAYERS",UDim2.fromOffset(24,50),UDim2.new(1,-48,0,18),8,Theme.Colors.White,Theme.Fonts.Strong).ZIndex=73
		local close=Button.new({Text="CLOSE",Variant="Secondary",Size=UDim2.fromOffset(96,34),OnActivated=function()overlay:Destroy()end});close.Position=UDim2.new(1,-120,0,20);close.ZIndex=74;close.Parent=hub
	...
	local function openTacticsHub()
		...
		text(hub,"SQUAD FORMATION  /  AI IDENTITY  /  MATCH BEHAVIOR",UDim2.fromOffset(24,50),UDim2.new(1,-48,0,18),8,Theme.Colors.White,Theme.Fonts.Strong).ZIndex=73
		local close=Button.new({Text="CLOSE",Variant="Secondary",Size=UDim2.fromOffset(96,34),OnActivated=function()overlay:Destroy()end});close.Position=UDim2.new(1,-120,0,20);close.ZIndex=74;close.Parent=hub
		...
		local function renderHub()
			...
			text(pitch,"STARTING XI SHAPE",UDim2.fromOffset(22,55),UDim2.new(1,-44,0,18),8,Theme.Colors.White,Theme.Fonts.Strong).ZIndex=75
			...
			for index,dot in dots do
				local node=Instance.new("Frame");node.AnchorPoint=Vector2.new(.5,.5);node.Position=UDim2.fromScale(dot[1],dot[2]);node.Size=UDim2.fromOffset(index==1 and 24 or 18,index==1 and 24 or 18);node.BackgroundColor3=index==1 and Theme.Colors.White or Theme.Colors.White;node.BorderSizePixel=0;node.ZIndex=76;node.Parent=field
			...
			local function drawSlider(name:string,row:number,column:number)
				...
				local fill=Instance.new("Frame");fill.BackgroundColor3=Theme.Colors.White;fill.BorderSizePixel=0;fill.Size=UDim2.fromScale(value/100,1);fill.ZIndex=77;fill.Parent=bar
				...
				local valueText=text(rowFrame,tostring(value),UDim2.new(1,-68,0,10),UDim2.fromOffset(32,20),8,Theme.Colors.White,Theme.Fonts.Display);valueText.TextXAlignment=Enum.TextXAlignment.Center;valueText.ZIndex=77
			...
			for _,category in categories do
				text(sliderList,category[1],UDim2.fromOffset(0,y),UDim2.new(1,0,0,18),8,Theme.Colors.White,Theme.Fonts.Strong).ZIndex=76
				y+=24
