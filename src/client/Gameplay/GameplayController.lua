...
function Controller:_state(payload:any)
	...
	elseif payload.Type=="Pass"then if self.HUD then self.HUD:HideKickoffScorer()end;if self.Ball then self.Ball.LocalTransparencyModifier=0 end;if self.Visual then self.Visual:PlayFlightTrail()end;local controller=self.AnimationCache and self.AnimationCache[payload.Actor];if controller then controller:Play("Pass")end;if payload.Actor==self.ActiveModel and self.Trainer and not UserInputService.TouchEnabled then self.Trainer:NotifyAction("Pass")end
	...
	elseif payload.Type=="Shot"then if self.CrowdAmbience then self.CrowdAmbience:Boost(0.9)end;if self.ReplayController then self.ReplayController:MarkShot(payload.Actor)end;local controller=self.AnimationCache and self.AnimationCache[payload.Actor];if controller then controller:Play("Shoot")end;if self.Visual then self.Visual:PlayShotTrail()end;if self.HUD then self.HUD:ShowShotChance(payload.ShotXG or payload.ScoringChance or payload.ScoringChancePercent,payload.Actor)end;if payload.Actor==self.ActiveModel and self.Trainer and not UserInputService.TouchEnabled then self.Trainer:NotifyAction("Shoot")end
