...
function Controller:_state(payload:any)
	...
	if payload.Type=="MatchStarted"then
		...
		else task.delay(2.5,function()if Players.LocalPlayer:GetAttribute("VTRInMatch")then self:_activate(payload)end end)end
		return
	end
	if not self.Active then return end
	...
	if payload.Type=="ActivePlayer"and payload.Model and payload.Model:IsA("Model")then self:_bindFootballer(payload.Model,payload.Name,payload.Position);if payload.Reason=="PenaltyDefense"then self.SetPieceMode="PenaltyDefense";self.MatchInPlay=false;if self.AimLine then self.AimLine:SetMatchActive(true)end;if self.GoalTarget then self.GoalTarget:SetMatchActive(true)end end
	...
	elseif payload.Type=="Possession"then self.HUD:SetPossession(payload.Owner or"",payload.OwnerUserId==Players.LocalPlayer.UserId);self.Indicators:SetBallCarrier(payload.Model);if payload.Model then if self.Visual then self.Visual:StopShotTrail()end;if self.GoalTarget then self.GoalTarget:Unlock()end end
	...
	elseif payload.Type=="SubSuggestion"then self.HUD:ShowSubSuggestion(payload,function(accepted:boolean)self.Action:FireServer({Type="SubstitutionResponse",Accepted=accepted})end)
