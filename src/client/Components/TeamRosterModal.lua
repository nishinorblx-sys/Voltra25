...
function TeamRosterModal.open(teamId: string, onError: (string) -> ())
	...
	label(modal, string.format("%s  /  %s  /  %s", data.Team.country, data.Team.league, data.Formation), UDim2.fromOffset(24, 50), UDim2.new(1, -48, 0, 18), 9, Theme.Colors.Electric, Theme.Fonts.Strong)
	...
	local function renderCards(players: { any }, emptyMessage: string)
		...
		scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; scroll.CanvasSize = UDim2.new(); scroll.ScrollBarThickness = 3; scroll.ScrollBarImageColor3 = Theme.Colors.Electric; scroll.ZIndex = 163; scroll.Parent = content
	...
	render = function()
		...
		if active == "FORMATION" then
			...
			local stroke = Instance.new("UIStroke"); stroke.Color = Theme.Colors.Electric; stroke.Transparency = .7; stroke.Thickness = 1; stroke.Parent = pitch
			...
			label(summary, "TEAM RATINGS", UDim2.fromOffset(16, 16), UDim2.new(1, -32, 0, 20), 9, Theme.Colors.Electric, Theme.Fonts.Strong)
			...
			label(summary, "STAR PLAYERS", UDim2.fromOffset(16, 230), UDim2.new(1, -32, 0, 20), 8, Theme.Colors.Electric, Theme.Fonts.Strong)
