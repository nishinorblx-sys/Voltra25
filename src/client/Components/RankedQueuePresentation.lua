...
local function base(root:Instance):CanvasGroup
	...
	local stroke=Instance.new("UIStroke");stroke.Color=Theme.Colors.White;stroke.Transparency=.72;stroke.Thickness=1;stroke.Parent=wash
...
function Presentation.StartSearching(root:Instance)
	...
	local scanner=Instance.new("Frame");scanner.AnchorPoint=Vector2.new(.5,.5);scanner.BackgroundColor3=Theme.Colors.White;scanner.BorderSizePixel=0;scanner.Position=UDim2.fromScale(.5,.47);scanner.Size=UDim2.fromOffset(250,2);scanner.ZIndex=246;scanner.Parent=overlay
	...
	TweenService:Create(scanner,TweenInfo.new(.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Size=UDim2.fromOffset(520,2),BackgroundTransparency=.25}):Play();TweenService:Create(core,TweenInfo.new(1.1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Rotation=225}):Play();TweenService:Create(title,TweenInfo.new(.85,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{TextColor3=Theme.Colors.White}):Play()
...
local function teamPanel(overlay:CanvasGroup,summary:any,side:string,controlledSide:string):Frame
	...
	local stroke=Instance.new("UIStroke");stroke.Color=side=="Home"and Theme.Colors.White or Color3.fromHex("D9D9D9");stroke.Transparency=.35;stroke.Thickness=1;stroke.Parent=panel
	...
	text(panel,tostring(summary.overall or 0),UDim2.fromScale(.34,.55),UDim2.fromScale(.32,.17),Theme.Fonts.Display,42,Theme.Colors.White,247)
...
function Presentation.ShowMatchFound(root:Instance,data:any,onComplete:()->())
	...
	text(overlay,"OPPONENT FOUND",UDim2.fromScale(.15,.08),UDim2.fromScale(.7,.07),Theme.Fonts.Display,29,Theme.Colors.White,246)
	...
	local vs=text(overlay,"VS",UDim2.fromScale(.44,.42),UDim2.fromScale(.12,.12),Theme.Fonts.Display,31,Theme.Colors.White,248);vs.TextTransparency=1
