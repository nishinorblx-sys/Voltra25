...
function Presentation.play(root:Frame,setup:any,home:any,away:any,onReturn:(()->())?,notify:((string,string)->())?,watchMode:boolean?)
	...
	preloadHandle=EssentialMatchPreloadService.Begin({world,setup,home,away,homeKit,awayKit,AnimationConfig},function(progress:number,processed:number,total:number)
		if not overlay or not overlay.Parent then return end
		fill.Size=UDim2.fromScale(math.clamp(progress,0,1),1)
		entering.Text=total>0 and string.format("ESSENTIAL ASSETS  %d / %d",processed,total)or"ESSENTIAL ASSETS READY"
	end)
	if not preloadHandle:Wait(6)then preloadHandle:Cancel()end
	Players.LocalPlayer:SetAttribute("VTREssentialMatchReady",true)
