...
function Editor.open(parent:Instance,identity:any,options:any)
	...
	text(modal,"LIVE KIT + BADGE GENERATOR  /  SERVER-VALIDATED COLORS",UDim2.fromOffset(24,48),UDim2.new(1,-48,0,18),8,Theme.Colors.Electric,Theme.Fonts.Strong)
	...
	local controls=Instance.new("ScrollingFrame");controls.BackgroundTransparency=1;controls.BorderSizePixel=0;controls.Position=UDim2.new(.36,8,0,82);controls.Size=UDim2.new(.64,-32,1,-164);controls.CanvasSize=UDim2.fromOffset(0,940);controls.ScrollBarThickness=3;controls.ScrollBarImageColor3=Theme.Colors.Electric;controls.ZIndex=212;controls.Parent=modal
	...
	renderOptions=function()
		...
		text(optionHolder,"BADGE SYMBOL  /  SELECT A MARK",UDim2.fromOffset(0,210),UDim2.new(1,0,0,24),8,Theme.Colors.Electric,Theme.Fonts.Strong)
		...
		for order,symbolName in BadgeSymbolLibrary.Symbols do
			...
			local selection=Instance.new("UIStroke");selection.Color=Theme.Colors.Electric;selection.Thickness=state.BadgeSymbol==symbolName and 2 or 1;selection.Transparency=state.BadgeSymbol==symbolName and 0 or .78;selection.Parent=choice
