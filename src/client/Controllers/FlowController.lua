...
function FlowController:ModeTransition(title:string,callback:()->(),compact:boolean?)
	...
	local slash=Instance.new("Frame");slash.AnchorPoint=Vector2.new(.5,.5);slash.BackgroundColor3=Theme.Colors.White;slash.BorderSizePixel=0;slash.Position=UDim2.fromScale(-.25,.5);slash.Rotation=-16;slash.Size=UDim2.fromScale(.55,1.7);slash.ZIndex=81;slash.Parent=overlay
...
function FlowController:_packResults(title:string,reveals:any,complete:()->())
	...
	local hint=titleLabel:Clone();hint.Position=UDim2.fromOffset(24,58);hint.Size=UDim2.new(1,-48,0,22);hint.Text="SELECT ANY CARD TO OPEN THE FULL PLAYER DATABASE PROFILE";hint.TextColor3=Theme.Colors.White;hint.TextSize=8;hint.Font=Theme.Fonts.Strong;hint.Parent=panel
