...
function FlowController:ModeTransition(title:string,callback:()->(),compact:boolean?)
	...
	TweenService:Create(overlay,TweenInfo.new(.16),{GroupTransparency=0}):Play();TweenService:Create(slash,TweenInfo.new(.36,Theme.Animation.EasingStyle,Theme.Animation.EasingDirection),{Position=UDim2.fromScale(1.22,.5)}):Play()
	task.delay(.18,callback);task.delay(compact and .3 or .42,function() TweenService:Create(overlay,TweenInfo.new(.16),{GroupTransparency=1}):Play();task.delay(.17,function() overlay:Destroy();self.Busy=false end) end)
end
