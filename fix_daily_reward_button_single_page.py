...
for path in (root/"src/client").rglob("*.lua"):
	...
	text=re.sub(
		r'\nlocal DailyLoginRewardPanel=require\([^\n]*DailyLoginRewardPanel[^\n]*\)\n',
		"\n",
		text
	)

	text=re.sub(
		r'\nlocal function vtrOpenDailyRewardOverlay\(context:any\?, parent:Instance\?\)[\s\S]*?local function vtrCreateDailyRewardTopButton\(parent:Instance, settingsButton:GuiObject\?, context:any\?\)[\s\S]*?end\n',
		"\n",
		text
	)

	text=re.sub(
		r'\n\s*vtrCreateDailyRewardTopButton\([^\n]*\)\n',
		"\n",
		text
	)
...
print("rewrote",button_path.as_posix())